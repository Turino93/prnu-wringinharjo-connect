...
-- Drop old permissive policies on letters table
DROP POLICY IF EXISTS "Anyone can view letters" ON public.letters;
...
DROP POLICY IF EXISTS "Anyone can delete letters" ON public.letters;

-- Create new policies: everyone can view, only admins can modify
...
CREATE POLICY "Admins can create letters"
ON public.letters
FOR INSERT
TO authenticated
WITH CHECK (public.has_role(auth.uid(), 'admin'));
